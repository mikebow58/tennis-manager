...
export async function handlePostCloseCancellation({
  ...
  // Will be recalculated after promotion in Case B to account for the
  // promoted player leaving the tentative pool.
  ...
  if (cancelledPlayerStatus === 'confirmed' && tentativePlayers.length > 0) {
    ...
    if (playerToPromote) {
      ...
      // Recalculate subsNeeded now that one tentative player has been promoted.
    // The promoted player's court now has one fewer tentative player.
