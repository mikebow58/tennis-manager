...
export async function sendSignupRequest({ playerName, playerEmail, signupUrl, weekLabel }) {
  // unchanged — still used for any one-off sends if needed
...
export async function sendCancellationNotice({ adminEmail, playerName, sessionDate, location }) {
  // unchanged — single recipient, no batch needed
  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to: adminEmail.includes(',') 
...
// Sends up to 100 pre-built email objects in a single Resend API call.
// Each item in the array must already be a complete Resend email object
// (from, to, subject, html). Returns { sent, failed } counts.
...
// Public helper: chunks an array of email objects into batches of 100
// and fires one Resend batch call per chunk. Returns { sent, failed } totals.
