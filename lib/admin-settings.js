...
//
// Four underlying tables/concepts are involved, even though this is
// conceptually "one settings page" to the organiser:
//   - admin_settings   (key-value store) -> admin_email,
//                                            include_roster_in_reminder
...
//
// THIS REVISION: added getIncludeRosterInReminder / setIncludeRosterInReminder
// (Communication section, post-beta item #5 — "Roster in reminder email").
// Unlike admin_email, this setting defaults to true (feature ON) when the
// key has never been written — per the Project Summary Section 21 spec,
// "Admin settings toggle in Communication section, default on." A missing
// row must not silently disable the feature on a fresh or pre-migration
// database.
...
// ---------------------------------------------------------------------------
// COMMUNICATION SETTINGS — admin_settings.include_roster_in_reminder
// ---------------------------------------------------------------------------

/**
 * Reads whether confirmed-tier reminder emails should include the alphabetical
 * roster of confirmed players (post-beta item #5). Stored as the text 'true'
 * or 'false' in admin_settings, matching the existing key-value pattern.
 *
 * DEFAULT-ON BEHAVIOUR (differs from getAdminEmail): per Project Summary
 * Section 21, this feature defaults to ON. A missing row (fresh database,
 * or a database that predates this feature) must not silently disable the
 * roster — it returns true, not null/false. A read error also fails open
 * to true for the same reason: this is a display feature, not a security
 * or capacity gate, so failing open costs nothing worse than an unexpected
 * (but harmless) roster line in an email.
 *
 * @returns {Promise<boolean>} true (include roster) by default; false only
 *   when the organiser has explicitly turned it off via the settings page.
 */
export async function getIncludeRosterInReminder() {
  const { data, error } = await supabaseAdmin
    .from('admin_settings')
    .select('value')
    .eq('key', 'include_roster_in_reminder')
    .maybeSingle()

  if (error) {
    console.error('getIncludeRosterInReminder error — defaulting to true (on):', error)
    return true
  }

  if (!data) {
    // Key has never been written — default on, per spec.
    return true
  }
...
/**
 * Upserts the roster-in-reminder toggle into admin_settings. Used by the
 * settings page API route (PUT /api/admin/settings).
 *
 * @param {boolean} value - true to include the roster, false to omit it.
 * @returns {Promise<boolean>} true on success, false on error.
 */
export async function setIncludeRosterInReminder(value) {
  const { error } = await supabaseAdmin
    .from('admin_settings')
    .upsert(
      { key: 'include_roster_in_reminder', value: value ? 'true' : 'false' },
      { onConflict: 'key' }
    )

  if (error) {
    console.error('setIncludeRosterInReminder error:', error)
    return false
  }
  return true
}