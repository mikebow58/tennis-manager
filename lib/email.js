...
export async function sendPostCloseCancellationAlert({
  ...
  // Resolve recipient with comma-separated support and dev override.
  ...
  // Describe the roster situation concisely for the subject line.
  const spotLabel = openSpots === 1 ? '1 spot open' : `${openSpots} spots open`

  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `Cancellation — ${cancelledPlayerName} · ${sessionDateLabel} · ${spotLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Post-close cancellation</h2>

        <p style="color: #444; line-height: 1.6;">
          <strong>${cancelledPlayerName}</strong> has been removed from
        ...
        <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin: 24px 0;">
          ...
          <div style="margin-bottom: 8px;">
            <span style="color: #888; font-size: 13px;">Open spots</span><br/>
            <span style="color: ${openSpots > 0 ? '#dc2626' : '#16a34a'}; font-weight: 500;">
              ${openSpots}
            </span>
          </div>
          <div>
            <span style="color: #888; font-size: 13px;">Sub request status</span><br/>
            <span style="color: #111;">${subRequestSummary}</span>
          </div>
        </div>

        ${openSpots > 0
          ? `<p style="color: #444; line-height: 1.6;">
              The system is looking for a replacement. You will receive a follow-up
              if a broadcast is sent or if no replacement is found by 5:00pm.
            </p>`
          : `<p style="color: #444; line-height: 1.6;">
              The session is still at full capacity. No replacement is needed.
            </p>`
        }
      </div>
    `,
