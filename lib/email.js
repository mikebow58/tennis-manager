...
export async function sendCourtCancellationNotice(players) {
  const emails = players.map((player) => ({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to: player.playerEmail,
    subject: `Session update — ${player.sessionDate}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Update on ${player.sessionDate}</h2>
        ...
        <p style="color: #444; line-height: 1.6;">
          We're sorry for the late notice. We hope to see you next week.
        </p>
      </div>
    `,
  }))
 ...
 * incomplete courts. Single organiser alert covering all cancellations
 * across a session.
 *
 * @param {object} params
 ...
 * @param {string} params.sessionDateLabel
 * @param {string} params.locationName
 * @param {number} params.cancelledCourtPlayerCount - How many tentative players were dropped
 * @param {number} params.confirmedCount            - Remaining confirmed players
 * @param {string[]} params.cancelledPlayerNames    - Names of players whose spots were cancelled
 * @returns {Promise<boolean>}
 */
export async function sendBackstopCancellationAlert({
  ...
  const droppedLabel = cancelledCourtPlayerCount === 1
    ? '1 player has been removed'
    : `${cancelledCourtPlayerCount} players have been removed`
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Auto-cancel fired — ${sessionDateLabel}</h2>

        <p style="color: #444; line-height: 1.6;">
          The 8:00pm backstop has fired for <strong>${sessionDateLabel}</strong>.
          The incomplete court could not be filled in time.
          <strong>${droppedLabel}</strong> from the roster and notified by email.
        </p>

        <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin: 24px 0;">
          <div style="margin-bottom: 8px;">
            <span style="color: #888; font-size: 13px;">Cancelled spots</span><br/>
            <span style="color: #111; font-weight: 500;">${cancelledPlayerNames.join(', ')}</span>
          </div>
          <div>
            <span style="color: #888; font-size: 13px;">Remaining confirmed players</span><br/>
            <span style="color: #111; font-weight: 500;">${confirmedCount}</span>
          </div>
        </div>

        <p style="color: #444; line-height: 1.6; font-size: 14px;">
          Court assignment details have been sent to all ${confirmedCount} remaining confirmed players.
          If you find a replacement after this point, add them manually via the admin dashboard.
        </p>
      </div>
    `,
  })
