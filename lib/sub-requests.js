...
async function evaluateAndSendSubRequest({
  ...
  // Resolve the response-deadline label once here so both the waitlist-first
  // branch below and the broadcastToAllAvailable fallthrough share the same
  // value instead of each re-fetching admin_settings independently.
  let effectiveDeadlineLabel = deadlineLabel
  if (!effectiveDeadlineLabel) {
    const { data: deadlineSetting } = await supabaseAdmin
      .from('admin_settings')
      .select('value')
      .eq('key', 'court_assignment_deadline')
      .maybeSingle()
    effectiveDeadlineLabel = formatDeadlineTime(deadlineSetting?.value ?? '20:00')
  }
    ...
    if (waitlistError) {
      ...
      if (waitlistInsertError || !newWaitlistRequest) {
        ...
        const waitlistRecipients = waitlistPlayers
          ...
          .map((p) => ({
            playerFirstName: p.players.first_name,
            playerEmail: p.players.email,
...
export async function broadcastToAllAvailable({
  ...
  // Same fallback pattern as evaluateAndSendSubRequest — this function is
  // also called directly by app/api/cron/waitlist-expiry/route.js, which
  // won't have a deadlineLabel on hand, so this keeps it safe to call
  // standalone without forcing every caller to resolve it first.
  let effectiveDeadlineLabel = deadlineLabel
  if (!effectiveDeadlineLabel) {
    const { data: deadlineSetting } = await supabaseAdmin
      .from('admin_settings')
