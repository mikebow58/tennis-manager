...
//
// THIS REVISION (Gap 3, Aug 30 2026 session): added
// sendAddedToWaitlistNotice — the player-facing email for a late
// sub-request respondent who claim_sub_request() silently placed on the
// waitlist instead of telling them the spot was already filled (Phase 3
// Group 3). See app/api/subs/respond/route.js's SUCCESS_WAITLISTED
// branch. No other function in this file changed.
 ...
 *
 * THIS REVISION (post-beta item #5 — "Roster in reminder email"): each
 * player object may optionally carry a `rosterNames` array — the
 * alphabetical (by first name) list of confirmed players for the day,
 * formatted as "First L.", built by app/api/cron/daily-8am/route.js Check B
 * Pass 2. When present and non-empty, a "Who's playing" section is rendered
 * between the session-details block and the notes/cancel-link footer. When
 * absent, undefined, or an empty array (organiser has the Communication
 * setting off, or the roster genuinely has zero confirmed players — which
 * shouldn't occur since this function is only ever called with confirmed
 * players in the first place), the section is omitted entirely — no empty
 * heading, no trailing whitespace artifact.
 *
...
export async function sendConfirmedReminderBatch(players) {
  const emails = players.map((player) => {
    const rosterHtml = Array.isArray(player.rosterNames) && player.rosterNames.length > 0
      ? `
        <div style="margin: 16px 0;">
          <span style="color: #888; font-size: 13px; display: block; margin-bottom: 6px;">
            Who's playing
          </span>
          <span style="color: #111; font-size: 14px; line-height: 1.6;">
            ${player.rosterNames.join(', ')}
          </span>
        </div>
      `
      : ''
 ...
 *
 * Never includes a roster — the tiered reminder system's own design
 * principle keeps this message status-only (Automation Logic §2.1). See
 * post-beta item #5 design decision: tentative stays minimal.
 *
 * Uses sendEmailBatch() which chunks into groups of 100 and applies
 * DEV_EMAIL_OVERRIDE automatically.
 *
...
// ---------------------------------------------------------------------------
// V2 — COURT ASSIGNMENT REVIEW (Dynamic Short vs Full Paths)
// ---------------------------------------------------------------------------

/**
 * Sends the organiser a court assignment review request at 6:00pm.
 * Dynamically adapts its wording if the day is perfectly full/balanced
 * versus if there are stragglers/incomplete courts.
 *
 * BUG FIX (dev session Aug 25, 2026): the body paragraph and warning banner
 * both previously said "add a player manually" / "not manually added a
 * player" unconditionally — singular, regardless of subsNeeded. When a
 * court was short by more than one player, this implied a single add would
 * resolve it, which is misleading. Both now use the same singular/plural
 * pattern already established by subsLabel just above.
 */
export async function sendCourtAssignmentReview({
  ...
  tentativePlayerNames,
  courtSummaryLines = [],
  reviewUrl, // absolute URL to /admin/court-assignment/[sessionId]
}) {
  const to = DEV_OVERRIDE
    ? DEV_OVERRIDE
    : adminEmail.includes(',')
  ...
  // 1. Determine if this day actually has incomplete courts
  const isShortSession = tentativeCount > 0 || subsNeeded > 0

  // 2. Dynamic Copy Elements
  const subsLabel = subsNeeded === 1 ? '1 player still needed' : `${subsNeeded} players still needed`

  // Shared singular/plural phrase for "add a player" / "add N players" —
  // used in both the body paragraph and the warning banner below, so the
  // two can't drift out of sync with each other or with subsLabel again.
  const addPlayersPhrase = subsNeeded === 1 ? 'a player' : `${subsNeeded} players`

  const subject = isShortSession
    ? `Court assignment — ${sessionDateLabel} · ${subsLabel} · action by ${deadlineLabel}`
    : `Court assignment ready — ${sessionDateLabel} · All courts full`

  const bodyParagraph = isShortSession
    ? `<strong>${sessionDateLabel}</strong> is still short at 6:00pm. 
       The automated broadcast window is now closed. 
       You have until <strong>${deadlineLabel}</strong> to add ${addPlayersPhrase} manually 
       before the incomplete court is auto-cancelled.`
    : `<strong>${sessionDateLabel}</strong> is fully arranged and balanced! 
       All confirmed players have been assigned to complete courts. 
       Please review the arrangements below before approving and notifying the players.`

  const atRiskHtml = isShortSession && tentativePlayerNames.length > 0
    ? `
      <p style="color: #444; line-height: 1.6; font-size: 14px;">
        If the court cannot be filled by ${deadlineLabel},
        <strong>${tentativePlayerNames.join(', ')}</strong>
  ...
  const warningBannerHtml = isShortSession
    ? `
      <div style="background: #fef2f2; border: 1px solid #fca5a5; border-radius: 8px; padding: 16px; margin: 16px 0;">
        <p style="color: #991b1b; font-size: 14px; margin: 0;">
          <strong>Auto-cancel at ${deadlineLabel}:</strong> If you have not manually added ${addPlayersPhrase}
  ...
  // Render court summary lines if Procedure 2 produced them.
  const courtSummaryHtml = courtSummaryLines.length > 0
    ? `
      <div style="margin: 16px 0;">
        <span style="color: #888; font-size: 13px; display: block; margin-bottom: 8px;">
          Recommended court arrangements
        </span>
        ${courtSummaryLines.map((line) => `
          <div style="padding: 8px 12px; background: #f9fafb; border-radius: 6px; margin-bottom: 6px; font-size: 14px; color: #111;">
            ${line}
          </div>
        `).join('')}
      </div>
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        ...
        <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin: 24px 0;">
          ...
          <div style="margin-bottom: 8px;">
            <span style="color: #888; font-size: 13px;">Roster</span><br/>
            <span style="color: #111; font-weight: 500;">${confirmedCount} confirmed · ${tentativeCount} tentative · ${isShortSession ? subsLabel : '0 players still needed'}</span>
          </div>
        </div>
...
export async function sendCourtCancellationNotice(players) {
  const emails = players.map((player) => ({
    ...
    html: `
      ...
      //  Commenting this out because this info provides no value to the player. Consider deleting entirely. 
       /* <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin: 24px 0;">
          <div style="margin-bottom: 8px;">
            <span style="color: #888; font-size: 13px;">Date</span><br/>
            <span style="color: #111; font-weight: 500;">${player.sessionDate}</span>
...
// ---------------------------------------------------------------------------
// V2 — COURT ASSIGNMENT DETAILS WITH COURT NUMBER
// (used by approval route and daily_8pm_backstop when court numbers are set)
// ---------------------------------------------------------------------------

/**
 * Sends full court assignment details to a batch of confirmed players,
 * including their real-world court number. Called when the organiser has
 * approved assignments AND assigned court numbers before the 8pm backstop,
 * or when the backstop fires and all courts have numbers set.
 *
 * This is the player-facing email they actually care about — it tells them
 * exactly which court to go to. Court letter is intentionally omitted from
 * this email; players only need the number.
 *
 * Optionally includes a rotation line (if this court is paired with another
 * for racquet rotation) and a freeform organiser note (if one was entered
 * for this court on the review screen). Both are short, plain-language
 * additions players already know how to interpret — see Project Summary
 * Section on court rotation pairing for the underlying mechanics.
 *
 * Uses sendEmailBatch() — DEV_EMAIL_OVERRIDE applied automatically.
 *
 * @param {Array<{
 ...
 *   courtNumber: number,
 *   notes: string|null,
 *   rotationLabel?: string|null,  // e.g. "Court A (winner's court), rotating
 *                                  //   with Court B. Switching partners each set."
 *   courtNote?: string|null,      // e.g. "Joe & John keep partners all day"
 *   cancelUrl: string
 * }>} players
 * @returns {Promise<{sent: number, failed: number}>}
 */
export async function sendCourtAssignmentDetailsFull(players) {
  const emails = players.map((player) => ({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to: player.playerEmail,
    subject: `Court assignment — ${player.sessionDate}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">You're on Court ${player.courtNumber}</h2>

        <p style="color: #444; line-height: 1.6;">
          Hi ${player.playerFirstName}, here are your details for tomorrow.
        </p>
        ...
        <div style="background: #f0fdf4; border: 1px solid #86efac; border-radius: 8px; padding: 16px; margin: 24px 0;">
          ...
          <div>
            <span style="color: #888; font-size: 13px;">Court</span><br/>
            <span style="color: #111; font-weight: 500; font-size: 18px;">Court ${player.courtNumber}</span>
          </div>
        </div>

        ${player.rotationLabel ? `
...
// ---------------------------------------------------------------------------
// V2 — PRE-CLOSE WAITLIST PROMOTION (used by lib/waitlist-promotion.js)
// ---------------------------------------------------------------------------

/**
 * Sent to a waitlisted player who has been promoted to confirmed after a
 * pre-close cancellation opened a spot. Pre-close context — no court or
 * time-sensitive deadline language needed, just confirmation that they're
 * now signed up.
 *
 * @param {object} params
 * @param {string} params.playerFirstName
 ...
 * @param {string} params.sessionDateLabel  - e.g. "Wednesday, May 21"
 * @param {string} params.locationName
 * @param {string} params.portalUrl         - Absolute URL to player's portal
 * @returns {Promise<boolean>}
 */
export async function sendWaitlistSpotOpenedToConfirmed({
  playerFirstName,
  playerEmail,
  sessionDateLabel,
  locationName,
  ...
  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `Good news — you're in! ${sessionDateLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Good news, ${playerFirstName}!</h2>
        <p style="color: #444; line-height: 1.6;">
          A spot opened up for <strong>${sessionDateLabel}</strong> at ${locationName},
          and you've been moved from the waitlist to the confirmed roster.
        </p>
        <p style="color: #444; line-height: 1.6;">
          You're all set — no action needed. We'll see you there!
        </p>
        <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #eee;">
          <a href="${portalUrl}" style="color: #888; font-size: 12px;">
            View or manage your signup
          </a>
        </div>
      </div>
    `
  ...
  if (error) {
    console.error('[email] sendWaitlistSpotOpenedToConfirmed error:', error)
    return false
  }
  return true
}
...
/**
 * Notifies the organiser that a pre-close cancellation triggered an
 * automatic waitlist promotion. Only sent when a PLAYER caused the
 * cancellation — not when the organiser removed the player themselves
 * (they already see the result immediately in the admin UI).
 *
 * @param {object} params
 * @param {string} params.adminEmail          - From getAdminEmail() (comma-sep OK)
 * @param {string} params.cancelledPlayerName
 * @param {string} params.promotedPlayerName
 * @param {string} params.sessionDateLabel
 * @param {string} params.locationName
 * @returns {Promise<boolean>}
 */
export async function sendWaitlistPromotionNotice({
  adminEmail,
  cancelledPlayerName,
  promotedPlayerName,
  sessionDateLabel,
  ...
  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `Waitlist promotion — ${sessionDateLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Automatic waitlist promotion</h2>
        <p style="color: #444; line-height: 1.6;">
          <strong>${cancelledPlayerName}</strong> cancelled their spot for
          <strong>${sessionDateLabel}</strong> at ${locationName}.
        </p>
        <p style="color: #444; line-height: 1.6;">
          <strong>${promotedPlayerName}</strong> was automatically promoted from
          the waitlist to fill the open spot. The session remains full — no
  ...
  if (error) {
    console.error('[email] sendWaitlistPromotionNotice error:', error)
    return false
  }
  return true
}
...
// ---------------------------------------------------------------------------
// V2 — LATE-ARRIVAL WAITLIST NOTICE (Gap 3 fix, Aug 30 2026 session — used
// by app/api/subs/respond/route.js, SUCCESS_WAITLISTED path only)
// ---------------------------------------------------------------------------

/**
 * Sent to a player who responded to a sub-request broadcast (first_call or
 * all_available) after every open spot had already been claimed by someone
 * else. Per Phase 3 Group 3, this player is added to the bottom of the
 * unified dynamic waitlist (FIFO by availability.created_at) rather than
 * turned away — and critically, per that same spec section, this email
 * must NOT reveal that the spot was already filled. The tone mirrors a
 * normal, positive confirmation: they responded, they've been placed on
 * the list, they'll hear back if something opens up. Contrast with
 * sendWaitlistSpotOpenedToConfirmed above, which is the opposite direction
 * (a waitlisted player being promoted TO confirmed) — this one is the
 * initial waitlist placement itself.
 *
 * Single-recipient send — resolveRecipient() applies DEV_EMAIL_OVERRIDE
 * the same way every other single-recipient player-facing function here
 * does.
 *
 * @param {object} params
 * @param {string} params.playerFirstName
 ...
 * @returns {Promise<boolean>}
 */
export async function sendAddedToWaitlistNotice({
  playerFirstName,
  playerEmail,
  sessionDateLabel,
  locationName,
  ...
  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `You're on the waitlist — ${sessionDateLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Thanks, ${playerFirstName}!</h2>
        <p style="color: #444; line-height: 1.6;">
          You're on the waitlist for <strong>${sessionDateLabel}</strong> at ${locationName}.
          We'll reach out right away if a spot opens up.
        </p>
        <p style="color: #444; line-height: 1.6;">
          No action needed for now — just keep an eye on your email.
        </p>
      </div>
    `,
  })
  ...
  if (error) {
    console.error('[email] sendAddedToWaitlistNotice error:', error)
    return false
  }
  return true
}
...
// ---------------------------------------------------------------------------
// V2 — ADMIN ADD RACE ALERT (used by app/api/admin/availability/route.js)
// ---------------------------------------------------------------------------

/**
 * Alerts the organiser when a manual post-close add lands as 'waitlisted'
 * instead of 'confirmed' because the incomplete court had already been
 * resolved by another path between when the organiser decided to add this
 * player and when the request was submitted. Mirrors the Phase 2 Section
 * 7.4 pattern for cancelled/declined → waitlisted override races.
 *
 * @param {object} params
 * @param {string} params.adminEmail        - From getAdminEmail() (comma-sep OK)
 * @param {string} params.addedPlayerName
 * @param {string} params.sessionDateLabel
 * @param {string} params.locationName
 * @returns {Promise<boolean>}
 */
export async function sendAdminAddRaceAlert({
  adminEmail,
  addedPlayerName,
  sessionDateLabel,
  locationName,
  ...
  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `Heads up — ${addedPlayerName} went to waitlist, not the roster`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Added to waitlist, not the roster</h2>
        <p style="color: #444; line-height: 1.6;">
          You added <strong>${addedPlayerName}</strong> to
          <strong>${sessionDateLabel}</strong> at ${locationName}, but the open spot
          had already been filled by the time the add went through.
        </p>
        <p style="color: #444; line-height: 1.6;">
          ${addedPlayerName} has been placed at the bottom of the waitlist instead.
          You'll want to contact them directly to explain the situation.
        </p>
      </div>
    `
  })
  ...
  if (error) {
    console.error('[email] sendAdminAddRaceAlert error:', error)
    return false
  }
  return true
}
...
// ---------------------------------------------------------------------------
// V2 — SUB REQUEST BROADCAST (real player-facing send, replaces
// sendSubRequestBroadcastStub — used by lib/sub-requests.js)
// ---------------------------------------------------------------------------

/**
 * Sends a real sub-request broadcast to a batch of targeted players, each
 * with a personal confirm/decline link. The link resolves to a GET page
 * (app/subs/respond/page.js) that shows session details and two buttons —
 * it does NOT mutate on load, so email-scanner prefetches (Outlook Safe
 * Links, Gmail link-scanning, etc.) cannot accidentally claim or decline
 * a spot on the player's behalf.
 *
 * Uses sendEmailBatch() — chunks into groups of 100, applies
 * DEV_EMAIL_OVERRIDE automatically.
 *
 * @param {Array<{
 *   playerFirstName: string,
 ...
 * @param {string} context.sessionDateLabel  - e.g. "Wednesday, May 21"
 * @param {string} context.locationName
 * @param {string} context.deadlineLabel     - e.g. "8:00pm" — when the
 *   broadcast window closes (6pm the day prior per Automation Logic
 *   Section 12.3 / Phase 3 Correction C-01)
 * @param {string|number} context.subRequestId
 * @returns {Promise<{sent: number, failed: number}>}
 */
export async function sendSubRequestBroadcast(recipients, {
  sessionDateLabel,
  locationName,
  deadlineLabel,
  subRequestId,
  ...
  const emails = recipients.map((player) => {
    const respondUrl =
      `${baseUrl}/subs/respond?token=${encodeURIComponent(player.signupToken)}&sub_id=${subRequestId}`

    return {
      from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
      to: player.email,
      subject: `Open spot — ${sessionDateLabel} at ${locationName}`,
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
          <h2 style="color: #111; margin-bottom: 8px;">Hi ${player.firstName},</h2>
          <p style="color: #444; line-height: 1.6;">
            A spot has opened up for <strong>${sessionDateLabel}</strong> at ${locationName}.
            Want it?
          </p>
          <div style="margin: 32px 0;">
            <a href="${respondUrl}"
          ...
          <p style="color: #444; font-size: 14px; margin-top: 24px;">
            Please respond by <strong>${deadlineLabel}</strong> — after that, this
            request closes and the spot may go to someone else.
          </p>
        </div>
      `,
    }
...
// ---------------------------------------------------------------------------
// V2 — SUB REQUEST RESOLVED NOTICE (organiser alert, used by
// app/api/subs/respond/route.js on full completion only — not on every
// individual confirm/decline, to avoid inbox noise)
// ---------------------------------------------------------------------------

/**
 * Notifies the organiser that a sub-request broadcast has been fully
 * resolved — every open spot has been claimed by a player through the
 * confirm/decline flow, and any previously tentative players have been
 * promoted to confirmed automatically.
 *
 * @param {object} params
 * @param {string} params.adminEmail        - From getAdminEmail() (comma-sep OK)
 * @param {string} params.sessionDateLabel
 * @param {string} params.locationName
 * @param {string} params.filledByPlayerName - The player whose confirm closed it out
 * @returns {Promise<boolean>}
 */
export async function sendSubRequestResolvedNotice({
  adminEmail,
  sessionDateLabel,
  locationName,
  filledByPlayerName,
  ...
  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `Court filled — ${sessionDateLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Court filled — ${sessionDateLabel}</h2>
        <p style="color: #444; line-height: 1.6;">
          <strong>${filledByPlayerName}</strong> confirmed the last open spot for
          <strong>${sessionDateLabel}</strong> at ${locationName}. The court is now
  ...
  if (error) {
    console.error('[email] sendSubRequestResolvedNotice error:', error)
    return false
  }
  return true
}
...
// ---------------------------------------------------------------------------
// V2 — ORGANISER-ADDED PLAYER REMOVAL ALERT (used by
// app/api/admin/availability/route.js DELETE, pre-close branch only)
// ---------------------------------------------------------------------------

/**
 * Sends an immediate alert to the organiser when a player removes themselves
 * PRE-CLOSE from a session the organiser had personally added them to
 * (availability.organiser_added = true). Ordinary pre-close removals stay
 * silent per Phase 2 Section 7.3 — this is the narrow exception carved out
 * for Project Summary Section 21's "Organiser-added player removal
 * notification" item: the organiser went out of their way to recruit this
 * player, so losing them without any signal is worth flagging.
 *
 * Standard post-close cancellation procedures already cover this player
 * once the session closes, regardless of how they originally joined the
 * roster — this alert exists only for the pre-close gap.
 *
 * @param {object} params
 * @param {string} params.adminEmail        - From getAdminEmail() (comma-sep OK)
 * @param {string} params.playerName        - e.g. "David Chen"
 * @param {string} params.sessionDateLabel  - e.g. "Wednesday, May 21"
 * @param {string} params.locationName      - e.g. "Memorial Park"
 * @returns {Promise<boolean>}
 */
export async function sendOrganiserAddedPlayerRemovedAlert({
  adminEmail,
  playerName,
  sessionDateLabel,
  locationName,
  ...
  if (DEV_OVERRIDE) {
    console.log(`[email] DEV_EMAIL_OVERRIDE active — sendOrganiserAddedPlayerRemovedAlert redirected to ${DEV_OVERRIDE}`)
  }

  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    subject: `${playerName} removed themselves — ${sessionDateLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">A player you added has backed out</h2>
        <p style="color: #444; line-height: 1.6;">
          <strong>${playerName}</strong> has removed themselves from
          <strong>${sessionDateLabel}</strong> at ${locationName}. You had personally
          added this player to the session, so we wanted to flag it directly rather
          than let it pass as routine roster churn.
        </p>
        <p style="color: #444; line-height: 1.6;">
          The session is still open for signups, so no other action has been taken
          automatically — this is just a heads-up in case you want to follow up with
          them or recruit a replacement.
        </p>
      </div>
    `,
  })
  ...
  if (error) {
    console.error('[email] sendOrganiserAddedPlayerRemovedAlert error:', error)
    return false
  }
  return true
}
// ---------------------------------------------------------------------------
// V2 — SESSION CANCELLATION NOTICE (used by
// app/api/sessions/[sessionId]/cancel/route.js)
// ---------------------------------------------------------------------------

/**
 * Sends a session cancellation notice to a batch of CONFIRMED players only.
 * Tentative and waitlisted players are deliberately excluded — per
 * organiser decision, they have already been told their status is
 * provisional and are not expecting to play unless/until told otherwise,
 * so a cancellation notice would be redundant rather than informative.
 *
 * Optionally includes a freeform cancellation reason (e.g. "Weather —
 * courts are wet"), stored on sessions.cancellation_note and passed through
 * unchanged. Reason is optional — omit the field entirely (or pass null)
 * for a plain cancellation notice.
 *
 * Uses sendEmailBatch() — chunks into groups of 100, applies
 * DEV_EMAIL_OVERRIDE automatically.
 *
 ...
 *   playerFirstName: string,
 *   playerEmail: string,
 *   sessionDateLabel: string,   - e.g. "Wednesday, May 21"
 *   locationName: string,
 *   cancellationNote: string|null
 * }>} players
 * @returns {Promise<{sent: number, failed: number}>}
 */
export async function sendSessionCancellationNotice(players) {
  const emails = players.map((player) => ({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to: player.playerEmail,
    subject: `Session cancelled — ${player.sessionDateLabel}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Your session has been cancelled</h2>

        <p style="color: #444; line-height: 1.6;">
          Hi ${player.playerFirstName}, unfortunately your tennis session on
          <strong>${player.sessionDateLabel}</strong> at ${player.locationName} has been cancelled.
        </p>

        ${player.cancellationNote ? `
        <div style="background: #fef2f2; border: 1px solid #fca5a5; border-radius: 8px; padding: 16px; margin: 16px 0;">
          <p style="color: #991b1b; font-size: 14px; margin: 0;">
            <strong>Reason:</strong> ${player.cancellationNote}
          </p>
        </div>` : ''}

        <p style="color: #444; line-height: 1.6;">
          We're sorry for any inconvenience — we'll see you at the next session.
        </p>
      </div>
    `,
  }))
...
// ---------------------------------------------------------------------------
// V2 — PLAYER CONTACT SUBMISSION (post-beta item #9 — "Player-to-organizer
// messaging"). Used by app/api/contact/route.js.
// ---------------------------------------------------------------------------

/**
 * Sends an immediate alert to the organiser when a player (or an
 * unverified visitor, via the broken-link fallback form) submits a contact
 * message. Single-recipient send — no batch needed. Per organiser decision,
 * this is an immediate-email-only notification; it does not also write to
 * the notifications table (matches Project Summary Section 5D's own
 * classification of this event type as an Immediate Email Notification).
 *
 * replyTo is set to the sender's email so the organiser can respond by
 * simply hitting reply in their email client — no separate lookup needed.
 *
 * @param {object} params
 * @param {string} params.adminEmail   - From getAdminEmail() (comma-sep OK)
 * @param {string} params.senderName   - Verified player's full name, or
 *   whatever name the anonymous visitor typed in
 * @param {string} params.senderEmail  - Verified player's email, or
 *   whatever email the anonymous visitor typed in
 * @param {string} params.message      - Freeform message text
 * @param {'verified'|'anonymous'} params.sourceLabel - which contact-form
 *   mode produced this submission (app/components/ContactForm.js)
 * @returns {Promise<boolean>}
 */
export async function sendPlayerContactSubmission({
  adminEmail,
  senderName,
  senderEmail,
  message,
  ...
  if (DEV_OVERRIDE) {
    console.log(`[email] DEV_EMAIL_OVERRIDE active — sendPlayerContactSubmission redirected to ${DEV_OVERRIDE}`)
  }

  const sourceBadge = sourceLabel === 'verified'
    ? 'Verified player (matched via their signup link)'
    : 'Unverified — typed their own name/email, could not be matched to a player record'

  const { error } = await resend.emails.send({
    from: 'Treviso - Memorial Park <noreply@gtcourts.com>',
    to,
    replyTo: senderEmail,
    subject: `Player message — ${senderName}`,
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">New message from a player</h2>

        <div style="background: #f9fafb; border-radius: 8px; padding: 16px; margin: 16px 0;">
          <div style="margin-bottom: 8px;">
            <span style="color: #888; font-size: 13px;">From</span><br/>
            <span style="color: #111; font-weight: 500;">${senderName} — ${senderEmail}</span>
          </div>
          <div>
            <span style="color: #888; font-size: 13px;">Source</span><br/>
            <span style="color: #111; font-weight: 500;">${sourceBadge}</span>
          </div>
        </div>
  ...
  if (error) {
    console.error('[email] sendPlayerContactSubmission error:', error)
    return false
  }
  return true
}