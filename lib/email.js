...
//
// @param {Array<{ playerName: string, playerEmail: string, signupUrl: string, portalUrl: string }>} players
//   Array of player objects. Each must have: playerName, playerEmail, signupUrl, portalUrl.
...
export async function sendSignupRequestBatch(players, weekLabel) {
  ...
  const emails = players.map((player) => ({
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto; padding: 24px;">
        <h2 style="color: #111; margin-bottom: 8px;">Hi ${player.playerName},</h2>
        <p style="color: #444; line-height: 1.6;">
