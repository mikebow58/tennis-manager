 ...
 *      availability records (court_letter, team_number,
 ...
 * every player object in playerPool (and therefore every downstream
 * court.players entry) only ever carried `skill`, the output of
 * resolveSkill() — which applies the internal-only cross-gender
 * comparison adjustment (-2 steps for female players; see
 * lib/court-balancing.js). That adjustment exists purely so the balancing
 * algorithm can compare skill levels fairly across genders — the codebase
 * convention (see daily-8am.js header note) is that it is "never stored
 * or displayed." The daily-6pm-court-assignment organiser email violated
 * that convention: it computed each complete court's displayed skill
 * range as getSkillLabel(min/max of court.players[].skill) — i.e. the
 * gender-adjusted value — with no other field available to use instead.
 * For a female player with skill_admin 1 or 2, the adjusted value goes to
 * -1 or 0, which has no valid NTRP label, producing garbage output like
 * "skill (-1--1)" and "skill (0-3.0-)" in the organiser email. Confirmed
 * live in the Sept 2 session's 6pm court assignment review for session 72.
 *
 * Fixed by adding a second field, `displaySkill`, alongside `skill` on
 * every player object — resolved the same way `skill` is (skill_admin,
 * falling back to mapped skill_self, default 4) but WITHOUT the gender
 * adjustment. `skill` continues to drive all balancing, court-letter
 * ordering, and team-pairing logic unchanged. `displaySkill` is the only
 * field the organiser-facing email (daily-6pm-court-assignment/route.js)
 * should ever read for a skill label. Both fields survive every spread
 * operation in this file (assignTeams, buildCourts) since they're carried
 * on the player object from the moment playerPool is built.
 *
 * References:
...
export async function runProcedure2(sessionId) {
  ...
  //
  // FIX (Sept 2): each player now carries BOTH `skill` (gender-adjusted,
  // for balancing/comparison only) and `displaySkill` (not gender-
  // adjusted, the only field safe to render via getSkillLabel() in any
  // player- or organiser-facing output). See file header for full
  // explanation.
  // ------------------------------------------------------------------
 ...
 *
 * availability: update court_letter, team_number, and court_assignment_status
 *   for each player. Confirmed players (complete courts) get status
 *   'confirmed'. Tentative players (incomplete courts) get status
 *   'tentative' and team_number null.
 *
 * displaySkill is a computed convenience field only — not written to
 * either table. Both tables continue to store skill_admin/skill_self on
 * the players row as the source of truth; nothing about this fix touches
 * persisted schema.
 *
...
async function writeAssignments(courts, daySessions) {
  ...
  // Update availability records — court_letter, team_number, and
  // court_assignment_status.
  //
